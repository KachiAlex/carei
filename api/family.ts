...
import { getSql, setCors, ensureTables, withTenant, getTenantSlug, getAuthToken } from './db.js'
import { verifyCredential, hashCredential, generateSecureToken, hashToken } from './hash.js'
...
async function getFamilyMemberFromToken(sql: any, token: string): Promise<any | null> {
  ...
  for (const row of rows) {
    const valid = await (await import('./hash.js')).verifyToken(token, row.token_hash)
    if (valid) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  FAMILY-AUTHENTICATED ENDPOINTS
    // ═══════════════════════════════════════════════════════════════
