...
import { fetchVisit, fetchClient, saveVisit, sendSOS, saveVisitDraft, getVisitDraft, getDrugInteractions, logMedication, getMedicationLogs, reportIncident, saveVoiceMemo, uploadFile, getHandoverBriefing, structureVoiceNotes, saveOutcomeIndicators } from '../api/client'
...
export default function ActiveVisitScreen() {
  ...
  // AI Voice-to-Documentation structuring
  const [aiStructuring, setAiStructuring] = useState(false)
  const [aiStructured, setAiStructured] = useState<any>(null)

  // Outcome Indicators
  const [outcomeMobility, setOutcomeMobility] = useState<number | null>(null)
  const [outcomeWellbeing, setOutcomeWellbeing] = useState<number | null>(null)
  const [outcomePain, setOutcomePain] = useState<number | null>(null)
  const [outcomeIndependence, setOutcomeIndependence] = useState<number | null>(null)

  const handleAIStructure = async () => {
    if (!transcript) return
    setAiStructuring(true)
    try {
      const res = await structureVoiceNotes({ transcript, visitId }) as any
      const structured = res.structured || res
      setAiStructured(structured)
      // Auto-fill visit fields from structured data
      if (structured.mood && !selectedMood) setSelectedMood(structured.mood)
      if (structured.appetite || structured.nutrition) setNutritionNote(structured.appetite || structured.nutrition)
      if (structured.notes) setNotes((n) => n ? n + '\n[AI] ' + structured.notes : '[AI] ' + structured.notes)
      if (structured.tasksCompleted && Array.isArray(structured.tasksCompleted)) {
        setTasks((prev) => prev.map((t) => {
          if (structured.tasksCompleted.some((tc: string) => t.name.toLowerCase().includes(tc.toLowerCase()))) {
            return { ...t, done: true, completedAt: Date.now() }
          }
          return t
        }))
      }
      if (structured.concerns || structured.handover) {
        const handoverText = [structured.concerns, structured.handover].filter(Boolean).join(' — ')
        setNotes((n) => n ? n + '\n[AI Handover] ' + handoverText : '[AI Handover] ' + handoverText)
      }
    } catch (err: any) {
      console.error('AI structure failed', err.message)
      alert('AI structuring failed: ' + err.message)
    } finally {
      ...
      <div className="flex-1 px-4 py-4 overflow-auto">
        ...
        {activeTab === 'notes' && (
          <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-3 mb-3">
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200">
              <div className="flex items-center justify-between mb-3">
                <div className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Outcome Indicators</div>
                <span className="text-[9px] text-slate-400">Structured scoring</span>
              </div>
              <div className="space-y-3">
                {/* Mobility */}
                <div>
                  <label className="text-[10px] font-medium text-slate-500 mb-1 block">Mobility (1=bedbound, 5=independent)</label>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button
                        key={n}
                        onClick={() => setOutcomeMobility(n)}
                        className="flex-1 py-1.5 rounded-lg text-xs font-semibold cursor-pointer border-none transition-all"
                        style={{
                          background: outcomeMobility === n ? `linear-gradient(90deg, ${COLORS.teal}, ${COLORS.teal2})` : '#f1f5f9',
                          color: outcomeMobility === n ? 'white' : '#64748b',
                        }}
                      >
                        {n}
                      </button>
                ...
                {/* Wellbeing Scale */}
                <div>
                  <label className="text-[10px] font-medium text-slate-500 mb-1 block">Wellbeing Scale (1=poor, 5=excellent)</label>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button
                        key={n}
                        onClick={() => setOutcomeWellbeing(n)}
                        className="flex-1 py-1.5 rounded-lg text-xs font-semibold cursor-pointer border-none transition-all"
                        style={{
                          background: outcomeWellbeing === n ? `linear-gradient(90deg, ${COLORS.teal}, ${COLORS.teal2})` : '#f1f5f9',
                          color: outcomeWellbeing === n ? 'white' : '#64748b',
                        }}
                      >
                        {n}
                      </button>
                ...
                {/* Pain Level */}
                <div>
                  <label className="text-[10px] font-medium text-slate-500 mb-1 block">Pain Level (0=none, 5=severe)</label>
                  <div className="flex gap-1">
                    {[0, 1, 2, 3, 4, 5].map((n) => (
                      <button
                        key={n}
                        onClick={() => setOutcomePain(n)}
                        className="flex-1 py-1.5 rounded-lg text-xs font-semibold cursor-pointer border-none transition-all"
                        style={{
                          background: outcomePain === n ? (n >= 3 ? COLORS.red : COLORS.amber) : '#f1f5f9',
                          color: outcomePain === n ? 'white' : '#64748b',
                        }}
                      >
                        {n}
                      </button>
                ...
                {/* Independence Level */}
                <div>
                  <label className="text-[10px] font-medium text-slate-500 mb-1 block">Independence Level (1=full support, 5=fully independent)</label>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button
                        key={n}
                        onClick={() => setOutcomeIndependence(n)}
                        className="flex-1 py-1.5 rounded-lg text-xs font-semibold cursor-pointer border-none transition-all"
                        style={{
                          background: outcomeIndependence === n ? `linear-gradient(90deg, ${COLORS.teal}, ${COLORS.teal2})` : '#f1f5f9',
                          color: outcomeIndependence === n ? 'white' : '#64748b',
                        }}
                      >
                        {n}
                      </button>
            ...
            {transcript && !isTranscribing && (
              <div className="mb-4">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl text-sm font-semibold text-white border-none cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                  style={{ background: `linear-gradient(90deg, #A78BFA, ${COLORS.teal})` }}
                >
                  {aiStructuring ? (
                    <>
                    ...
                    <>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l1.9 5.8a2 2 0 0 0 1.3 1.3L21 12l-5.8 1.9a2 2 0 0 0-1.3 1.3L12 21l-1.9-5.8a2 2 0 0 0-1.3-1.3L3 12l5.8-1.9a2 2 0 0 0 1.3-1.3L12 3z"/></svg>
                      Structure with AI
                    </>
                  )}
                </button>
                {aiStructured && (
                  <div className="mt-2 p-3 rounded-xl bg-purple-50 text-xs text-slate-600 space-y-1">
                    {aiStructured.mood && <div><strong>Mood:</strong> {aiStructured.mood}</div>}
                    {aiStructured.appetite && <div><strong>Appetite:</strong> {aiStructured.appetite}</div>}
                    {aiStructured.tasksCompleted && <div><strong>Tasks:</strong> {aiStructured.tasksCompleted.join(', ')}</div>}
                    {aiStructured.concerns && <div><strong>Concerns:</strong> {aiStructured.concerns}</div>}
                    <div className="text-purple-600 italic mt-1">Fields auto-filled from AI analysis</div>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={async () => {
                  ...
                  // Save outcome indicators if any were scored
                  if (outcomeMobility || outcomeWellbeing || outcomePain != null || outcomeIndependence) {
                    try {
                      await saveOutcomeIndicators({
                        visitId: visit.id,
                        clientId: client.id,
                        mobilityScore: outcomeMobility || undefined,
                        wellbeingScale: outcomeWellbeing || undefined,
                        painLevel: outcomePain ?? undefined,
                        independenceLevel: outcomeIndependence || undefined,
                      })
                    } catch (err: any) { console.error('saveOutcomeIndicators failed', err.message) }
                  }
