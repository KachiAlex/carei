...
export default function ManagerDashboard() {
  ...
  const renderOverview = () => (
    <motion.div className="flex flex-col gap-3" initial="hidden" animate="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.06 } } }}>
      ...
      {/* AI Feature Links */}
      <motion.div custom={7} variants={cardVariants} className="grid grid-cols-4 gap-2">
        {[
          { label: 'AI Reports', path: `/tenant/${tenantSlug}/manager/reports`, color: '#A78BFA' },
          { label: 'Compliance', path: `/tenant/${tenantSlug}/manager/compliance`, color: COLORS.teal },
          { label: 'Risk Alerts', path: `/tenant/${tenantSlug}/manager/risk-alerts`, color: COLORS.red },
          { label: 'Staff Match', path: `/tenant/${tenantSlug}/manager/staff-matching`, color: COLORS.amber },
        ].map((link) => (
          <button
            key={link.label}
            onClick={() => setLocation(link.path)}
