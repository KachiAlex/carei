...
export default function SplashScreen() {
  ...
  const isNative = useState(() => {
    ...
    if (window.location.protocol.startsWith('capacitor')) return true
  ...
  if (loading) {
    return <LoadingOverlay onDone={async () => {
      const token = getToken() || await secureGet('token')
      if (token) {
        setLocation('/select-tenant')
      } else {
        const onboardingComplete = localStorage.getItem('carei_onboarding_complete')
